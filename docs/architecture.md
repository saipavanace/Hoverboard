...
| **admin_persisted_snapshot** | Legacy unused table in some DBs; not written by the app for mirror export (reserved / inert) |
...
All application behavior reads **relational tables** (`drs`, `vrs`, `coverage_metrics`, …). The **Data mirror** in **Admin** is a **read-only** JSON export via **`GET /api/admin/full-snapshot`**; it does not write back to the database. See **[Administrator guide — Full data mirror](admin_guide.md#full-data-mirror-system-administrator)**.
